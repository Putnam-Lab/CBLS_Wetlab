...
#month <-daily_measurements[daily_measurements$Date >= "2022-06-01" & daily_measurements$Date <= "2023-09-01", ] # use if you would like to subset by month or from certain date range 
...
temp <- ggplot(daily_measurements, aes(x=Date, y=Temperature_C, colour = Tank_ID)) + 
    geom_line() + 
    theme_classic() +
...
sal <- ggplot(daily_measurements, aes(x=Date, y=Salinity_psu, colour = Tank_ID)) + 
    geom_line() + 
    theme_classic() + 
      scale_x_date(date_labels = "%b-%Y", date_breaks = "2 months") +
    geom_point() +
...
path <-("../Data/tris_cal/") #set path to calibration files
...
print(tris$Temperature_C)
mvTris <- tris$Temperature_C*pH.cals$Slope+pH.cals$Intercept #calculate the mV of the tris standard using the temperature mv relationships in the measured standard curves 
STris<-35 #salinity of the Tris
print(mvTris)

phTris<- (11911.08-18.2499*STris-0.039336*STris^2)*(1/(tris$Temperature_C+273.15))-366.27059+ 0.53993607*STris+0.00016329*STris^2+(64.52243-0.084041*STris)*log(tris$Temperature_C+273.15)-0.11149858*(tris$Temperature_C+273.15) #calculate the pH of the tris (Dickson A. G., Sabine C. L. and Christian J. R., SOP 6a)
print(phTris)

pH.Total<-phTris+(mvTris/1000-tris$pH_mv/1000)/(R*(tris$Temperature_C+273.15)*log(10)/F) #calculate the pH on the total scale (Dickson A. G., Sabine C. L. and Christian J. R., SOP 6a)

print(pH.Total)

library(lubridate)
# populate empty dataframe with Tris calibrations 
tris$light1 <- pH.Total

total_pH$pH.Total <- pH.Total
total_pH$Date <- tris$Date
Tank_ID <- tris$Tank_ID
total_pH$Tank_ID <- tris$Tank_ID
...
# updated this script 5/12/23 LZ
# export the total_pH dataframe as an output csv

write.table(total_pH,"../Data/total_pH_output.csv",sep=",", row.names=FALSE) 

# read in cumulative total_pH 

cumu.pH <- read.csv("../Data/total_pH.csv")

update.pH <- rbind(cumu.pH, total_pH) # bind most recent total_pH to cumulative total pH dataframe 

tail(update.pH) #check that most recent total_pH measurements have been appended to the cumu dataframe

# export data as csv file
write.table(update.pH,"../Data/total_pH.csv",sep=",", row.names=FALSE)
...
TA_data <- read.csv("../Data/CBLS_Aquarium_TA.csv")
...
pdf(file = "../Output/20230830_dailymeasurements.pdf",
    width = 4,
    height = 4)

# combine plots using ggpubr command onto one page

daily <- ggarrange(temp, sal, TA, 
          ncol = 1, nrow = 3) #removed total pH for now
