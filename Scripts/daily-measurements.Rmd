...
pHSlope <- pHSlope %>%
  select(Date, Tank_ID, Time, Date.Time, Observer.Initials, Temperature_C, Salinity_psu, pH.total, TA, pH_mv, Orion_Temp, light2, light3, light4, tris.date, Probe.Set, notes, `(Intercept)`, TTris, mVTris)
```
...
hlines_data <- list(
  ...
  data.frame(yintercept = 36, metric = "Salinity_psu"), # upper threshold for salinity in psu
  ...
  data.frame(yintercept = 2500.000, metric = "TA") # upper threshold for total pH
    )
```
